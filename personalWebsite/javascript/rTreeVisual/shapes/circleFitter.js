...
function makeCorcle(points) {
   ...
   shuffled.forEach((p, i) => {
       if (c === null || !isInCorcle(c, p))
           c = makeCorcleOnePoint(shuffled.slice(0, i + 1), p);
   });
   return c;
}
// One boundary point known
function makeCorcleOnePoint(points, p) {
   let c = new Corcle(p.x, p.y, 0);
   points.forEach((q, i) => {
       if (!isInCorcle(c, q)) {
           if (c.r == 0)
               c = makeDiameter(p, q);
           else
               c = makeCorcleTwoPoints(points.slice(0, i + 1), p, q);
       }
   });
   return c;
}
// Two boundary points known
function makeCorcleTwoPoints(points, p, q) {
   const circ = makeDiameter(p, q);
   let left = null;
   let right = null;
   // For each point not in the two-point circle
   for (const r of points) {
       if (isInCorcle(circ, r))
           continue;
...
function makeDiameter(a, b) {
   ...
   return new Corcle(cx, cy, Math.max(r0, r1));
}
function makeCircumcircle(a, b, c) {
   ...
   return new Corcle(x, y, Math.max(ra, rb, rc));
}
/* Simple mathematical functions */
const MULTIPLICATIVE_EPSILON = 1 + 1e-14;
function isInCorcle(c, p) {
